' ===== Generates an report for Total Admitted, Total Present & Percentage =====
Sub GenerateAttendanceReportFormatted()
    ...
    ' Sorting for Array("Year", "Branch", "Division") - Year: Custom, Branch: Alphabetical, Division: Alphabetical
    sortingDict("Year-Branch-Division") = Array("FE,SE,TE,BE", "ALPHABETICAL", "ALPHABETICAL")
    ...
    For Each groupCols In groupCombos
        ...
        For row = 2 To lastRow
            ...
            If Not reportDict.exists(key) Then
                reportDict(key) = Array(0, 0)
            End If
            
            ' Fix: Extract array, modify, then store back
        ...
        reportWS.Cells(outputRow, 4).Value = "Percentage"
        reportWS.Range(reportWS.Cells(outputRow, 1), reportWS.Cells(outputRow, 4)).Font.Bold = True
        ...
        reportWS.Cells(outputRow, 4).Interior.Color = RGB(240, 240, 240) ' Light gray for calculated percentage
        ...
        For Each k In sortedKeys
            ...
            ' Make Total Admitted and Total Present editable values
            reportWS.Cells(outputRow, 2).Value = tempArray(0) ' Total Admitted (editable)
            reportWS.Cells(outputRow, 3).Value = tempArray(1) ' Total Present (editable)
            
            ' Add background colors to indicate editable columns
            reportWS.Cells(outputRow, 2).Interior.Color = RGB(255, 255, 200) ' Light yellow for Total Admitted
            reportWS.Cells(outputRow, 3).Interior.Color = RGB(200, 255, 200) ' Light green for Total Present
            
            ' Formula for Percentage - Present out of Admitted
            reportWS.Cells(outputRow, 4).Formula = "=IF(" & reportWS.Cells(outputRow, 2).Address & ">0," & reportWS.Cells(outputRow, 3).Address & "/" & reportWS.Cells(outputRow, 2).Address & ",""N/A"")"
            reportWS.Cells(outputRow, 4).NumberFormat = "0.00%"
            reportWS.Cells(outputRow, 4).Interior.Color = RGB(240, 240, 240) ' Light gray for calculated
        ...
        reportWS.Range(reportWS.Cells(outputRow, 1), reportWS.Cells(outputRow, 4)).Font.Bold = True
        reportWS.Range(reportWS.Cells(outputRow, 2), reportWS.Cells(outputRow, 4)).Interior.Color = RGB(220, 220, 220) ' Darker gray for totals
    ...
    reportWS.Range(reportWS.Cells(outputRow, 1), reportWS.Cells(outputRow, 4)).Font.Bold = True
    reportWS.Range(reportWS.Cells(outputRow, 2), reportWS.Cells(outputRow, 4)).Interior.Color = RGB(200, 200, 200) ' Dark gray for grand total
    ...
    reportWS.Cells(outputRow + 2, 1).Value = "• Gray cells (Percentages & Totals) - Auto-calculated"
    reportWS.Cells(outputRow + 3, 1).Value = "• Change any editable value and percentages will update automatically"
    reportWS.Cells(outputRow + 4, 1).Value = "• Groups are sorted according to predefined rules (see settings)"
    ...
    MsgBox "Smart Attendance Report with Custom Sorting generated successfully!" & vbCrLf & vbCrLf & _
           "Features:" & vbCrLf & _
           "• Total Admitted (Yellow) - Editable" & vbCrLf & _
           "• Total Present (Green) - Editable" & vbCrLf & _
           "• Percentages (Gray) - Auto-calculated" & vbCrLf & _
